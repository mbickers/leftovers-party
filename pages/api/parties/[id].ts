...
const parseForm = async (req: NextApiRequest) => {
  ...
  { fields: Fields, files: Files, err?: any }
  >((resolve, reject) => {
...
export default async function handler(
  ...
  const existingParty = await prisma.party.findUnique(
    { where: { id: partyId }, include: { leftovers: true } },
  );
  if (!existingParty) {
    res.status(404).json(errorBody(`invalid id '${partyId}'`));
    return;
  }
  ...
  if (req.method === 'POST') {
    const { fields, files: uploadedImages } = await parseForm(req);
    ...
    const newPhotoPaths = await Promise.all(
      newLeftovers.map(({ id }) => photos.store(uploadedImages[id] as File)),
    );
    ...
    await Promise.all([...leftoversToDelete.map((leftover) => {
      const photoName = leftover.image_url.split('/api/photos/')[1];
      return photos.remove(photoName);
    })]);

    const out = await prisma.$transaction([
      prisma.leftover.deleteMany({ where: { id: { in: leftoversToDelete.map(({ id }) => id) } } }),
      ...leftoversToUpdate.map(({ id, owner, description }) => prisma.leftover.update({
        where: { id }, data: { owner, description },
      })),
      ...leftoversToCreate.map((({ owner, description, image_url }) => prisma.leftover.create({
        data: {
          owner, description, image_url, partyId,
        },
      }))),
      prisma.party.update({ include: { leftovers: true }, where: { id: partyId }, data: { name } }),
    ]);
